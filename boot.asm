;Created by CyberCraft Studio in 5.06.2024
...
input_loop: ;опис функції input_loop
    ...
    cmp al, 'v'     ;перевірка, що в регістрі al символ r
    je check_for_ver;перескакуємо до мітки check_for_ver
...
check_for_ver:
    mov ah, 0       ; Читання з клавіатури без відображення на екрані
    int 16h         ; Виклик послуги BIOS для введення з клавіатури
    cmp al , 'e'    ;перевірка, що в регістрі al символ e
    jne input_loop  ;якщо ні, вертаємося до мітки input_loop

    mov ah, 0       
    int 16h         ;Виклик послуги BIOS для введення з клавіатури
    cmp al, 'r'     ;перевірка, що в регістрі al символ r
    jne input_loop  ;якщо ні, вертаємося до мітки input_loop
